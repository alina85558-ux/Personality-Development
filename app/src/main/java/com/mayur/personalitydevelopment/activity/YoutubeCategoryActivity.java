...
import static com.mayur.personalitydevelopment.connection.ApiCallBack.allYoutube;
...
public class YoutubeCategoryActivity extends BaseActivity implements ExerciseAdapter.AdapterListener {
    ...
    public static void start(Context context, int courseCategoryId, int categoryId, String course) {
        Intent starter = new Intent(context, YoutubeCategoryActivity.class);
    ...
    public void getCourses() {
        try {
            ...
            connectPost(this, null, allYoutube(BaseActivity.getKYC(), authToken, sp.getBoolean("guest_entry", false), Constants.getV6Value(), categoryId, course), new ApiConnection.ConnectListener() {
                @Override
                public void onResponseSuccess(String response, Headers headers, int StatusCode) {
                    InnerYoutubeCategoryResponse data = new Gson().fromJson(response, InnerYoutubeCategoryResponse.class);
                    Utils.hideDialog();
                    progressBar.setVisibility(View.GONE);

                    courseCategoriesAdapter = new ExerciseAdapter(getBaseContext(), data.getMusics(), YoutubeCategoryActivity.this);
    ...
    @Override
    public void onClickEvent(YoutubeItem courseCategory) {

        Intent intent = new Intent(this, ExerciseActivity.class);
        intent.putExtra("selectedExercise", courseCategory);
