...
import static com.mayur.personalitydevelopment.connection.ApiCallBack.favArticleList;
...
public class FavouriteActivity extends BaseActivity implements ConnectivityReceiver.ConnectivityReceiverListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        LinearLayoutManager manager = new LinearLayoutManager(FavouriteActivity.this, LinearLayoutManager.VERTICAL, false);
    ...
    void setUpAdapter() {
        try {
            //if (restored_Issubscribed) {
                articleListAdapter = new ArticleListAdapter(articlesBeen, this, null, 4);
            ...
            /*} else {
                MobileAds.initialize(this, initializationStatus -> {
                    ...
                    articleListAdapter = new ArticleListAdapter(articlesBeen, FavouriteActivity.this, null, 4);
                    binding.lvMovies.setAdapter(articleListAdapter);
                });
            }*/
        } catch (Exception e) {
    ...
    private void showArticleOptionToWatchDialog(final int articleID) {
        ...
        btn_subscribe.setOnClickListener(view -> {
            dialog.dismiss();
            startActivity(new Intent(FavouriteActivity.this, RemoveAdActivity.class));
        });
...
//            Appodeal.disableLocationPermissionCheck();
            ...
            Appodeal.initialize(FavouriteActivity.this, APP_KEY, Appodeal.REWARDED_VIDEO, list -> {
                //Appodeal initialization finished
                Appodeal.cache(FavouriteActivity.this, Appodeal.REWARDED_VIDEO);
                ...
                if (loadAndShow) {
                    if (Appodeal.isLoaded(Appodeal.REWARDED_VIDEO)) {
                        Appodeal.show(FavouriteActivity.this, Appodeal.REWARDED_VIDEO);
                    }
                }
            });
    ...
    public void callArticleIntent(Articles currentSelectedArticle) {
        Intent intent = new Intent(FavouriteActivity.this, ArticleDetailActivity.class);
    ...
    public void updateWatchedVideoStatus(int articleID) {
        ...
        try {
            String authToken = "";
            if (Constants.getUserData(FavouriteActivity.this) != null) {
                authToken = Constants.getUserData(FavouriteActivity.this).getAuthentication_token();
            }

            Log.e("authToken: ", authToken + " token");

            connectPost(FavouriteActivity.this, null, unlockArticle(BaseActivity.getKYC(), authToken, false, Constants.getV6Value(), articleID, true),
                    new ApiConnection.ConnectListener() {
                        ...
                        public void onResponseFailure(ResponseBody responseData, Headers headers, int StatusCode) {
                            // Toast.makeText(FavouriteActivity.this, responseData.getMessage(), Toast.LENGTH_LONG).show();
                        }

                        @Override
                        public void onFailure(Headers headers) {
                            Toast.makeText(FavouriteActivity.this, "Failure", Toast.LENGTH_LONG).show();
                        }

                        @Override
                        public void onConnectionFailure() {
                            Toast.makeText(FavouriteActivity.this, "CC Failure", Toast.LENGTH_LONG).show();
                        }

                        @Override
                        public void onException(Headers headers, int StatusCode) {
                            Log.e("onException: ", headers.toString() + "");
                            Toast.makeText(FavouriteActivity.this, "EE Failure", Toast.LENGTH_LONG).show();
                        }
                    });
        } catch (Exception e) {
            e.printStackTrace();
    ...
    private void setOffLineData() {
        try {
            if (restored_Issubscribed) {
                ...
                binding.refreshLayout.setRefreshing(false);
                isLoading = false;
                ArticleRoomDatabase db = ArticleRoomDatabase.getDatabase(FavouriteActivity.this);
                if (db != null) {
                    List<Article> offlineArticles = db.articleDao().getBookMarkArticle(true);
    ...
    public void getFavourite() {
        ...
        try {
            ...
            connectPost(FavouriteActivity.this, null, favArticleList(BaseActivity.getKYC(), authToken, sp.getBoolean("guest_entry", false), Constants.getV6Value(), "" + current_page), new ApiConnection.ConnectListener() {
                @Override
                public void onResponseSuccess(String response, Headers headers, int StatusCode) {
                ...
                public void onException(Headers headers, int StatusCode) {
                    ...
                    binding.progress.setVisibility(View.GONE);
                    Utils.hideDialog();
                    Toast.makeText(FavouriteActivity.this, "EE Failure" + StatusCode, Toast.LENGTH_LONG).show();
                }
            });
        } catch (Exception e) {
            e.printStackTrace();
    ...
    private void insertArticle(List<Articles> articles) {
        try {
            if (articles != null && !articles.isEmpty()) {
                ArticleRoomDatabase articleRoomDatabase = ArticleRoomDatabase.getDatabase(FavouriteActivity.this);
        ...
        try {
            ...
            if (list != null && !list.isEmpty()) {
                ...
                if (ids != null && ids.length() > 0) {
                    updateArticleFavorites(ids, bookMarkStatus, isRefresh, articleIds);
                } else {
    ...
    public void updateArticleFavorites(String articleIds, String favoritsStatus, final boolean isRefresh, final ArrayList<String> favSynchList) {
        try {
            String authToken = "";
            if (Constants.getUserData(FavouriteActivity.this) != null) {
                authToken = Constants.getUserData(FavouriteActivity.this).getAuthentication_token();
            ...
            connectPost(FavouriteActivity.this, null, ApiCallBack.multipleArticleFavorite(BaseActivity.getKYC(), authToken, sp.getBoolean("guest_entry", false), articleIds, Constants.getV6Value(), favoritsStatus), new ApiConnection.ConnectListener() {
                @Override
                public void onResponseSuccess(String response, Headers headers, int StatusCode) {
                    Log.i(TAG, "onResponseSuccess: Sync Article Favorite Updated");
                    ArticleRoomDatabase db = ArticleRoomDatabase.getDatabase(FavouriteActivity.this);
                    if (favSynchList != null && favSynchList.size() > 0) {
                        for (int i = 0; i < favSynchList.size(); i++) {
                            db.articleDao().setSynch(false, Integer.parseInt(favSynchList.get(i)));
                        }
                    }
    ...
    private void createNativeAd() {
        try {
            ...
            MobileAds.initialize(FavouriteActivity.this, initializationStatus -> {
                Log.d(TAG, "Google SDK Initialized");

                AdLoader adLoader = new AdLoader.Builder(FavouriteActivity.this, getString(R.string.native_adv_ids))

                        .forNativeAd(nativeAd -> {
                            Log.d(TAG, "Native Ad Loaded");
                            if (FavouriteActivity.this.isDestroyed()) {
                                nativeAd.destroy();
...
//                                            this.articlesBeen.add(j,nativeAdList.get(i));
                                            articlesBeen.add(j,nativeAdList.get(i));
                                        }
                                    }
                                }
